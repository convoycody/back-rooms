...
Deno.serve(async (req) => {
  try {
    ...
    const { draw_id, use_vault } = await req.json();
    ...
    // Get draw
    const draw = await base44.entities.PoolDraw.get(draw_id);
    if (!draw) {
    ...
    if (config.pool_max_tickets_per_player) {
      const existingTickets = await base44.entities.PoolTicket.filter({
        player_id: player.id,
        draw_id: draw_id
      });
      
      if (existingTickets.length >= config.pool_max_tickets_per_player) {
        return Response.json({ 
          error: `Max ${config.pool_max_tickets_per_player} tickets per player` 
        }, { status: 400 });
      }
    }
    ...
    const existingTickets = await base44.entities.PoolTicket.filter({ draw_id: draw_id });
    const ticketNumber = existingTickets.length + 1;

    // Create ticket
    const ticket = await base44.asServiceRole.entities.PoolTicket.create({
      player_id: player.id,
      draw_id: draw_id,
